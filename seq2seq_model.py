...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    with tf.variable_scope(self.scope_name):
        ...
        self.learning_rate_set_op = self.learning_rate.assign(self.learning_rate * 20)
  ...
  def step_dual(self,session, buckets, encoder_inputs, decoder_inputs, target_weights,batch_source_encoder, batch_source_decoder, bucket_id,du_model,rev_vocab):
    def softmax(x):
    ...
    inv_encoder_inputs, inv_decoder_inputs, inv_target_weights, inv_batch_source_encoder, inv_batch_source_decoder = self.inverse(batch_source_encoder,resp_tokens,bucket_id)
    _,_,output_logits = du_model.step(session, inv_encoder_inputs, inv_decoder_inputs, inv_target_weights,bucket_id,forward_only=True, force_dec_input=True)

    logits_t = np.transpose(output_logits, (1,0,2))
    sum_p = []
    sum_tmp = []
    for i, (tokens, logits) in enumerate(zip(batch_source_encoder, logits_t)):
        p = 1
        ...
        if p < 1e-100:
          #print ("p: ", p)
          p = 1e-100
        if len(tokens)==0 or len(logits)==0:
          p=-1
        else:
          p = log(p) / len(tokens)
        #print ("logProb: p: %s" %(p))
        sum_p.append(-0.2*p+0.8)
        sum_tmp.append(1.)
    #print(sum_p)
    r4 = sum_p
    ep_rewards=[]
    ep_rewards.append(r4)
    advantage = ep_rewards * len(buckets)
    _, step_loss, _ = self.step(session, init_inputs[0], init_inputs[1], init_inputs[2], init_inputs[3],forward_only=False, force_dec_input=True, advantage=advantage)
    ansloss=[]
  ...
  def step_rl(self, session, buckets, encoder_inputs, decoder_inputs, target_weights, batch_source_encoder,batch_source_decoder, bucket_id, rev_vocab=None, debug=False,disSession=None,disModel=None):
    ...
    while True:
      ...
      for mmm in range(6):
        ...
        batch_size = disModel.config['batch_size']
        for i in range(disModel.config['batch_size']):
          d1_len = min(disModel.config['data1_maxlen'], len(batch_source_encoder[i]))
          d2p_len = min(disModel.config['data2_maxlen'], len(resp_tokens[(i+mmm)%batch_size]))
          d2n_len = min(disModel.config['data2_maxlen'], len(resp_tokens[(i+mmm)%batch_size]))
          X1[i*2,   :d1_len],  X1_len[i*2]   = batch_source_encoder[i][:d1_len],   d1_len
          X2[i*2,   :d2p_len], X2_len[i*2]   = resp_tokens[(i+mmm)%batch_size][:d2p_len], d2p_len
          X1[i*2+1, :d1_len],  X1_len[i*2+1] = batch_source_encoder[i][:d1_len],   d1_len
          X2[i*2+1, :d2n_len], X2_len[i*2+1] = resp_tokens[(i+mmm)%batch_size][:d2n_len], d2n_len
        ...
        for ll in pred:
          if iii%2==0:
            reward.append(float(ll))
          iii +=1
        base.append(reward)
      r4=[]
      for mmm in range(len(base[0])):
        summ=0.
        for lll in range(len(base)):
          summ+=base[lll][mmm]
        r4.append(10*base[0][mmm]/summ)
      #r4 = 1/(1+np.exp(-r4))
      #print("r4: %s" %(r4))
      R=[]
      for mmm in range(len(r3)):
        rr = -0.5*r3[mmm] + 0.5*r4[mmm] -3
        R.append(rr)
      ep_rewards.append(R)
      episode += 1
    defaltvec=[]
...
# log(P(b|a)), the conditional likelyhood
  ...
  def inverse(self, batch_source_encoder, batch_source_decoder,bucket_id):
    encoder_size, decoder_size = self.buckets[bucket_id]
    encoder_inputs, decoder_inputs = [], []
    for batch_i in xrange(self.batch_size):
      encoder_input = batch_source_decoder[batch_i]
      decoder_input = batch_source_encoder[batch_i]
    ...
    return batch_encoder_inputs, batch_decoder_inputs, batch_weights, batch_source_decoder, batch_source_encoder
  ...
  def caldummy(self, session, buckets, encoder_inputs, decoder_inputs, target_weights, batch_source_encoder,batch_source_decoder, bucket_id, rev_vocab=None):
    ...
    #print("r1: final value: ", r1)
    return r1
  def calprob(self, session, buckets, encoder_inputs, decoder_inputs, target_weights,batch_source_encoder, batch_source_decoder, bucket_id,du_model,rev_vocab):
    def softmax(x):
      return np.exp(x) / np.sum(np.exp(x), axis=0)
    init_inputs = [encoder_inputs, decoder_inputs, target_weights, bucket_id]
    encoder_state, step_loss, output_logits = self.step(session, encoder_inputs, decoder_inputs, target_weights,bucket_id, forward_only=True, force_dec_input=False)
