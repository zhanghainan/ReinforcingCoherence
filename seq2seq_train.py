...
tf.app.flags.DEFINE_string("train_dir2", "./dualOpenSubData/", "Training directory.")
...
tf.app.flags.DEFINE_boolean("dual_learning", True, "Turn on/off dual_learning")
...
def create_model(session, dummy_set,forward_only,dual):
  ...
  model = seq2seq_model.Seq2SeqModel(
      ...
      FLAGS.learning_rate, FLAGS.learning_rate_decay_factor, FLAGS.fixed_rate, FLAGS.weibo_rate, FLAGS.qa_rate,forward_only=forward_only,scope_name='seq2seq'+str(dual))
  if dual ==True:
    ckpt = tf.train.get_checkpoint_state(FLAGS.train_dir2)
  else:
        ...
        while True:
          ...
          for ind in range(30):
            ...
            for kk in range(500):
              ...
              inv_encoder_inputs, inv_decoder_inputs, inv_target_weights, inv_batch_source_encoder, inv_batch_source_decoder = model.inverse(batch_source_encoder, batch_source_decoder,bucket_id)
              if FLAGS.reinforce_learning:
                  if FLAGS.dual_learning:
                    _,step_loss1,_=model.step_dual(sess, _buckets, encoder_inputs, decoder_inputs, target_weights,batch_source_encoder, batch_source_decoder, bucket_id,du_model,rev_vocab=rev_vocab)
                    _,step_loss2,_=du_model.step_dual(sess, _buckets, inv_encoder_inputs, inv_decoder_inputs, inv_target_weights,inv_batch_source_encoder, inv_batch_source_decoder, bucket_id,model,rev_vocab=rev_vocab)
                    step_loss = []
                    for ii in range(len(step_loss1)):
                      step_loss.append(step_loss1[ii]+step_loss2[ii])
                  else:
              ...
              if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                checkpoint_path2 = os.path.join(FLAGS.train_dir2, "weibo.du_model")
                du_model.saver.save(sess, checkpoint_path2, global_step=model.global_step)
